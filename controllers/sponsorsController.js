...
module.exports = {
  getSponsors: async (req, res) => {
    try {
      const sponsors = await Sponsor.find({ isMain: true });
  ...
  postSponsor: async (req, res) => {
    try {
      if (Object.keys(req.body).length !== 0) {
        ...
        // Validate "link" attribute
        if (!req.body.link || !/^(http:\/\/|https:\/\/)/.test(req.body.link)) {
          return res
            .status(422)
            .json({
              message:
                "Invalid link format, it should start with http:// or https://",
            });
        }
